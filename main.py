...
class YouTubeDownloader:
    ...
    def download(self, url):
        ...
        ydl_opts = {
            ...
            # Playlist Handling
            'noplaylist': False,
            'ignoreerrors': True,  # CAUTION: This causes extract_info to return None on failure
            ...
            # JS Engine: REMOVED to allow auto-detection
            # 'jsengine': 'node',
        ...
        try:
            ...
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    # 1. Extract info (without downloading first to check validity)
                    info = ydl.extract_info(url, download=False)

                    # --- FIX: Handle failed extraction when ignoreerrors=True ---
                    if info is None:
                        print(f"\n[Error] Could not retrieve information for: {url}")
                        print("This often happens if the video is private, deleted, or signature solving failed.")
                        return False
            ...
            # This captures the generic Python errors
            print(f"\n[Critical Error] {e}")
            import traceback
            traceback.print_exc()  # Helps debug specific crashes
            return False
